...
# Train the model with best presets
bio01_model_best = TabularPredictor(
    label = 'BIO01',
    path = gd_dir + 'MAT_best',
    ).fit(
        hyperparameters = xgb_gpu_hyperparameters,
        train_data = bio01c_train,
        presets = 'best',
        time_limit= 3600*2
        )
# Train the model with extreme presets
bio01_model_extreme = TabularPredictor(
  label = 'BIO01',
  path = gd_dir + 'MAT_extreme',
  ).fit(
      train_data = bio01c_train,
      presets = 'extreme',
      time_limit= 3600*2
...
# Print and export the performances
reg_extra_metrics = ['root_mean_squared_error', 'mean_squared_error', 'mean_absolute_error', 'median_absolute_error', 'mean_absolute_percentage_error', 'r2', 'symmetric_mean_absolute_percentage_error']
bio01_best_results = evaluate_and_save_model(
    model=bio01_model_best,
    model_name="Bio01c_best",
    test_data=bio01c_test,
    output_dir=gd_dir,
    extra_metrics=reg_extra_metrics
)
bio01_extreme_results = evaluate_and_save_model(
    model=bio01_model_extreme,
    model_name="Bio01c_extreme",
    test_data=bio01c_test,
    output_dir=gd_dir,
    extra_metrics=reg_extra_metrics
)
